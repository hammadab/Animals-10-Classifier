...
def get_dataset(stage):
    ...
    # Read dataset files
    folders = [x[1] for x in os.walk('/content/drive/My Drive/CS 464-1 Introduction to Machine Learning/HW3/dataset')] # walk through everything in dataset
    ...
    for i, folder_name in enumerate(folders): # for each folder in dataset
        ...
        j = 0
        for filename in glob.glob("/content/drive/My Drive/CS 464-1 Introduction to Machine Learning/HW3/dataset/" + str(folder_name) + "/*"): # for each file in that folder
            j += 1
            if j <= 0.7 * 200: # training
                if stage == "training":
                    data.append([np.transpose(np.array(cv2.imread(filename)), (2, 0, 1)) / 255, np.array([i])]) # load that image and convert it to numpy array, add the class index for this image
...
def main(): # you are free to change parameters
    ...
    for epoch in range(0, max_epoch):
        train(epoch, model, criterion, optimizer, DataLoader(AnimalDataset("training"), batch_size=2, shuffle=True), device)
        acc = test(model, DataLoader(AnimalDataset("validation"), batch_size=2, shuffle=False), device)
        if acc > best_acc:
            best_acc = acc
            print("Best validation accuracy", acc)
            torch.save(model, "/content/drive/My Drive/CS 464-1 Introduction to Machine Learning/HW3/CNN_best_modelxCPU.pth")
...
def train(epoch, model, criterion, optimizer, loader, device): # you are free to change parameters
    ...
    if epoch % 10 == 9:
        ...
        print("Epoch: " + str(epoch+1) + " accuracies: " + str(accuracies/14) + " losses: " + str(losses))# + " batch_time: " + str(batch_time))
...
since = time.time()
main()
print("CPU Time elapsed:", (time.time() - since))